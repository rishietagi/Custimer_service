...
erDiagram
    ...
    users ||--o{ call_sessions : conducts
    call_sessions }o--o| appointments : creates
...
Stores simulation call sessions, transcripts, and structured call summaries.
* **call_id** (TEXT, PK): Unique voice call session ID (e.g., `CALL-XXXX`).
* **user_id** (TEXT, FK references `users.user_id` ON DELETE CASCADE)
* **start_time** (TEXT): Timestamp when the call starts.
* **end_time** (TEXT, Nullable): Timestamp when the call ends.
* **transcript** (TEXT, Nullable): Stringified JSON list of voice dialogue turns between user and agent.
* **summary** (TEXT, Nullable): Structured call summary generated by the LLM (Llama 3.3).
* **appointment_id** (TEXT, FK references `appointments.appointment_id` ON DELETE SET NULL, Nullable): Associated scheduled appointment.
* **status** (TEXT): Call state (`Active`, `Completed`, `Ended`).
